...
def partial_pivoting(A_aug: np.ndarray, row_num: int, col_num: int) -> np.ndarray:
    max_row = row_num
    max_val = abs(A_aug[row_num, col_num])

    for i in range(row_num, A_aug.shape[0]):
...
def gaussian_elimination(A: np.ndarray, b: np.ndarray) -> np.ndarray:
    ...
    for i in range(rows - 1, -1, -1):
        x[i] = (A_aug[i, -1] - (A_aug[i, i + 1 : cols] @ x[i + 1 :])) / A_aug[i, i]
